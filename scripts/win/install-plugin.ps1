...
$pluginName = "RhinoFSharpTemplate"
...
$builtPluginFile = Join-Path $buildOutput "RhinoFSharpTemplate.rhp"
$devInstallRoot = [IO.Path]::GetFullPath((Join-Path $repoRoot "bin\RhinoFSharpTemplateDev"))
$devInstallDirectory = [IO.Path]::GetFullPath((Join-Path $devInstallRoot "rh$RhinoMajorVersion"))
$pluginFile = Join-Path $devInstallDirectory "RhinoFSharpTemplate.rhp"
...
$developmentFiles = @(
    $builtPluginFile
    (Join-Path $buildOutput "RhinoFSharpTemplate.pdb")
    (Join-Path $buildOutput "RhinoFSharpTemplate.deps.json")
    (Join-Path $buildOutput "RhinoFSharpTemplate.runtimeconfig.json")
)
...
if (
    ...
    Write-Warning "Replacing the existing RhinoFSharpTemplate registration at '$existingPluginFile'. Uninstall any Package Manager copy to prevent it from reclaiming this plugin GUID."
}
...
elseif (
    ...
    if (Test-Path -LiteralPath $registryPath) {
        Write-Host "Removing incomplete per-user RhinoFSharpTemplate registration."
        Remove-Item -LiteralPath $registryPath -Recurse -Force
    }

    Write-Host "Using the complete machine-wide RhinoFSharpTemplate registration."
    ...
    throw "Rhino $RhinoMajorVersion has no completed RhinoFSharpTemplate registration. Open Rhino, install '$pluginFile' once through Options > Plug-ins, close Rhino, then run build-and-install.ps1 again."
}
