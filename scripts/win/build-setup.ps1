...
$project = Join-Path $projectRoot "RhinoFSharpTemplate.fsproj"
...
if (-not $MatrixOnly -and $requestedVersion -eq 0 -and -not [string]::IsNullOrWhiteSpace($env:RHINO_FSHARP_TEMPLATE_RHINO_VERSION)) {
    $environmentVersion = 0

    if (-not [int]::TryParse($env:RHINO_FSHARP_TEMPLATE_RHINO_VERSION.Trim(), [ref] $environmentVersion)) {
        throw "RHINO_FSHARP_TEMPLATE_RHINO_VERSION must be an integer, not '$($env:RHINO_FSHARP_TEMPLATE_RHINO_VERSION)'."
    }
